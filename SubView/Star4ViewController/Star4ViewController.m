...
@interface Star4ViewController ()
{
    ...
    BOOL firstOpen;
    //2/5
    GloBalAnalysisViewController *star4AnalysisSubView;
    float barHeight ;
    BOOL isFirstShake;    
}
@end

@implementation Star4ViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self)
    {
        ...
        [self setUI];
        isSmall = YES;       
        [self resetNumArray:STAR4_PER_NUM];
        [self makeBulrView];
...
{
    [DEFAULTS setObject:[NSNumber numberWithInt:FOUR_BALL_COUNT] forKey:@"AnalysisCellBallCount"];
    [DEFAULTS synchronize];
    sender.selected = YES;
    star4AnalysisSubView.playType = [NSString stringWithFormat:@"%d",DBTYPE_STAR_4];
    star4AnalysisSubView.dataArray = [countArray mutableCopy];
    [self presentViewController:star4AnalysisSubView animated:YES completion:^
     {
         sender.selected = NO;
     }];
...
-(void)star4Spin
{
    if (spinSameNum)
    {
        for (int i = 0; i<10; i++)
        {
            int rand = (arc4random() % STAR4_PER_NUM);
...
-(void)setNumBall
{
    numBallTwo = [[UIImageView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2 - pWidth(100), SCREEN_HEIGHT*0.3, pWidth(90), pWidth(90))];
    numBallThree = [[UIImageView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2 + pWidth(10), SCREEN_HEIGHT*0.3, pWidth(90), pWidth(90))];
    float middelLeft = CGRectGetMinX(numBallTwo.frame);
    float middelRight = CGRectGetMaxX(numBallThree.frame);
    numBallOne = [[UIImageView alloc] initWithFrame:CGRectMake(middelLeft - pWidth(75) - pWidth(5), SCREEN_HEIGHT*0.4, pWidth(75), pWidth(75))];
    numBallFour = [[UIImageView alloc] initWithFrame:CGRectMake(middelRight + pWidth(5), SCREEN_HEIGHT*0.4, pWidth(75), pWidth(75))];
...
-(void)saveBallRecord
{
    NSArray *firstThreeArray = [numArray objectsAtIndexes:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(0, 4)]];
...
-(void)teachYouHowToPlay
{
    ...
    {
        readyShakePhone.transform = CGAffineTransformMakeRotation(DEGREES_TO_RADIANS(-10));
        [self dismissReadArrow];
     }];
    
    
}
-(void)dismissReadArrow
{
    currentImageView.image = [currentImageView.image normalize];
    [SINGLETON fadeOutWithView:currentImageView];
...
{
    if (star4AnalysisSubView == nil)
    {
        star4AnalysisSubView = [[GloBalAnalysisViewController alloc] init];
        star4AnalysisSubView.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
        star4AnalysisSubView.modalPresentationStyle = UIModalPresentationFullScreen;
    }
}
...
- (void)didChangeStatusBarFrame:(NSNotification*)notification
{
    ...
    CGRect dataAnalyzeFrame = dataAnalyze.frame;
        ...
        dataAnalyzeFrame.origin.y = SCREEN_HEIGHT-(52+statusBarHeight);
        [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^
         {
             ...
             [dataAnalyze setFrame:dataAnalyzeFrame];
         } completion:nil];
}
@end
