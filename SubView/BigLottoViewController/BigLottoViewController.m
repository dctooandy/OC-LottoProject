...
@interface BigLottoViewController ()
{
    BOOL disMode;
    ...
    GloBalAnalysisViewController *bigLottoAnalysisSubView;
    //2/6
    UIImageView *currentImageView;
}
@end

@implementation BigLottoViewController
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    //刪除三步驟
    //號碼消除
    [self resetNumArray:BigLottoBallCount];
    //樹消除
    [self makeTree];
    //球消除
    [self hideNumBall];
...
-(void)dataAnalyze:(UIButton *)sender
{
    ...
    sender.selected = YES;
    bigLottoAnalysisSubView.playType = [NSString stringWithFormat:@"%d",DBTYPE_BIG_LOTTO];
    bigLottoAnalysisSubView.dataArray = [countArray mutableCopy];
    [self presentViewController:bigLottoAnalysisSubView animated:YES completion:^
     {
         sender.selected = NO;
     }];
}
...
//    treeImageView.contentMode = UIViewContentModeScaleAspectFill;
}
-(void)shakeBefore
{
...
-(void)bigLottoSpin
{
    int totalNum = BigLottoBallCount;
    if (disMode == YES)
    {
        NSMutableArray * newNumArray = [[NSMutableArray alloc] init];
        for (int i = 1; i <= BigLottoBallCount; i++)
        {
    ...
    }else
    {
        ...
        {
            int rand = (arc4random() % BigLottoBallCount);
            if (i!=rand)
            {
...
{
    if (bigLottoAnalysisSubView == nil)
    {
        bigLottoAnalysisSubView = [[GloBalAnalysisViewController alloc] init];
        bigLottoAnalysisSubView.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
        bigLottoAnalysisSubView.modalPresentationStyle = UIModalPresentationFullScreen;
    }
}
