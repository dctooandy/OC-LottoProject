...
#import "Constant.h"
@interface Thirty9ViewController ()
{
    ...
    GloBalAnalysisViewController *thirty9AnalysisSubView;
    //2/6
    UIImageView *currentImageView;
}
@end

@implementation Thirty9ViewController
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    //刪除三步驟
    //號碼消除
    [self resetNumArray:THIRTY9_PER_NUM];
    //樹消除
    [self makeTree];
    //球消除
    [self hideNumBall];
...
-(void)setUI
{
    ...
    [self makeTree];
    currentNumberImageView.image = [UIImage imageNamed:@"7_number of times.png"];
    [self setNumberInNumImageView:FOUR_BALL_COUNT];
...
-(void)dataAnalyze:(UIButton *)sender
{
    ...
    sender.selected = YES;
    thirty9AnalysisSubView.playType = [NSString stringWithFormat:@"%d",DBTYPE_THREE_NINE];
    thirty9AnalysisSubView.dataArray = [countArray mutableCopy];
    [self presentViewController:thirty9AnalysisSubView animated:YES completion:^
     {
         sender.selected = NO;
     }];
}
...
#pragma mark -
#pragma mark 39樂合彩 開始甩
-(void)thirty9Spin
{
    for (int i = 0; i<THIRTY9_PER_NUM; i++)
    {
        int rand = (arc4random() % THIRTY9_PER_NUM);
        if (i!=rand)
        {
...
-(void)showNumBall
{
    ...
    [UIView animateWithDuration:0.3
                     ...
                     animations:^
     {
         numBallOne.alpha =(([numArray objectAtIndex:0]==[firstSixArray objectAtIndex:5])||([numArray objectAtIndex:0]==[firstSixArray objectAtIndex:4]))?0.0: 1.0;
         numBallTwo.alpha =(([numArray objectAtIndex:1]==[firstSixArray objectAtIndex:5])||([numArray objectAtIndex:1]==[firstSixArray objectAtIndex:4]))?0.0: 1.0;
         numBallThree.alpha =(([numArray objectAtIndex:2]==[firstSixArray objectAtIndex:5])||([numArray objectAtIndex:2]==[firstSixArray objectAtIndex:4]))?0.0: 1.0;
         numBallFour.alpha =(([numArray objectAtIndex:3]==[firstSixArray objectAtIndex:5])||([numArray objectAtIndex:3]==[firstSixArray objectAtIndex:4]))?0.0: 1.0;
         numBallFive.alpha =(([numArray objectAtIndex:4]==[firstSixArray objectAtIndex:5])||([numArray objectAtIndex:4]==[firstSixArray objectAtIndex:4]))?0.0: 1.0;
         numBallSix.alpha =(([numArray objectAtIndex:5]==[firstSixArray objectAtIndex:5])||([numArray objectAtIndex:5]==[firstSixArray objectAtIndex:4]))?0.0: 1.0;
     } completion:^(BOOL finished)
     {
...
{
    if (thirty9AnalysisSubView == nil)
    {
        thirty9AnalysisSubView = [[GloBalAnalysisViewController alloc] init];
        thirty9AnalysisSubView.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
        thirty9AnalysisSubView.modalPresentationStyle = UIModalPresentationFullScreen;
    }
}
