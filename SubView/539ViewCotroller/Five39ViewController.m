...
#import "Constant.h"
@interface Five39ViewController ()
{
    ...
    GloBalAnalysisViewController *five39AnalysisSubView;
    //2/6
    UIImageView *currentImageView;
}
@end

@implementation Five39ViewController
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    //刪除三步驟
    //號碼消除
    [self resetNumArray:FIVE39_PER_NUM];
    //樹消除
    [self makeTree];
    //球消除
    [self hideNumBall];
...
-(void)setUI
{
    ...
    [self makeTree];
    currentNumberImageView.image = [UIImage imageNamed:@"5_number of times.png"];
...
-(void)dataAnalyze:(UIButton *)sender
{
    ...
    sender.selected = YES;
    five39AnalysisSubView.playType = [NSString stringWithFormat:@"%d",DBTYPE_FIVE_THREE_NINE];
    five39AnalysisSubView.dataArray = [countArray mutableCopy];
    [self presentViewController:five39AnalysisSubView animated:YES completion:^
     {
         sender.selected = NO;
     }];
}
...
#pragma mark -
#pragma mark 今彩539 開始甩
-(void)five39Spin
{
    for (int i = 0; i<FIVE39_PER_NUM; i++)
    {
        int rand = (arc4random() % FIVE39_PER_NUM);
        if (i!=rand)
        {
...
//        NSLog(@"1 : %@",[numArray objectAtIndex:0]);
//        NSLog(@"2 : %@",[numArray objectAtIndex:1]);
//        NSLog(@"3 : %@",[numArray objectAtIndex:2]);
//        NSLog(@"4 : %@",[numArray objectAtIndex:3]);
//        NSLog(@"5 : %@",[numArray objectAtIndex:4]);
//        NSLog(@"6 : %@",[numArray objectAtIndex:5]);
...
//    NSLog(@"firstSixArray :%@",firstSixArray);
    [UIView animateWithDuration:0.3
                          delay:0
...
{
    if (five39AnalysisSubView == nil)
    {
        five39AnalysisSubView = [[GloBalAnalysisViewController alloc] init];
        five39AnalysisSubView.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
        five39AnalysisSubView.modalPresentationStyle = UIModalPresentationFullScreen;
    }
}
