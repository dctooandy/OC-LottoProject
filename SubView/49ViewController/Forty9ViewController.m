...
#import "Constant.h"
@interface Forty9ViewController ()
{
    ...
    GloBalAnalysisViewController *forty9AnalysisSubView;
    //2/6
    UIImageView *currentImageView;
}
@end

@implementation Forty9ViewController
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    //刪除三步驟
    //號碼消除
    [self resetNumArray:FORTY9_PER_NUM];
    //樹消除
    [self makeTree];
    //球消除
    [self hideNumBall];
...
-(void)dataAnalyze:(UIButton *)sender
{
    ...
    sender.selected = YES;
    forty9AnalysisSubView.playType = [NSString stringWithFormat:@"%d",DBTYPE_FOUR_NINE];
    forty9AnalysisSubView.dataArray = [countArray mutableCopy];
    [self presentViewController:forty9AnalysisSubView animated:YES completion:^
     {
         sender.selected = NO;
     }];
}
...
-(void)forty9Spin
{
    for (int i = 0; i<FORTY9_PER_NUM; i++)
    {
        int rand = (arc4random() % FORTY9_PER_NUM);
        if (i!=rand)
        {
...
{
    if (forty9AnalysisSubView == nil)
    {
        forty9AnalysisSubView = [[GloBalAnalysisViewController alloc] init];
        forty9AnalysisSubView.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
        forty9AnalysisSubView.modalPresentationStyle = UIModalPresentationFullScreen;
    }
}
